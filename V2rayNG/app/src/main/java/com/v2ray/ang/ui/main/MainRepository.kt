...
class MainRepository(
        ...
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            val event = when (safeIntent.getIntExtra("key", 0)) {
                ...
                AppConfig.MSG_STATE_START_FAILURE -> MainServiceEvent.StateStartFailure(
                    safeIntent.getStringExtra("content")?.takeIf { it.isNotBlank() }
                )
    ...
    override fun encodeSubscription(id: String, item: SubscriptionItem) =
        MmkvManager.encodeSubscription(id, item)

    override fun removeSubscription(id: String) =
