...
/**
 * Zero VPN bottom navigation — premium liquid-goo bar, a 1:1 port of the
 * Android ZeroBottomNav: two anti-aliased neon capsules plus a
 * tangent-continuous cubic-bezier metaball neck drawn on the Canvas, with
 * bouncy spring travel between tabs.
 */
enum class ZeroTab { HOME, LOCATIONS, SETTINGS }
...
fun ZeroBottomNav(modifier: Modifier = Modifier) {
    val selectedTab = when (Store.view) {
        "locations" -> ZeroTab.LOCATIONS
        "settings" -> ZeroTab.SETTINGS
        else -> ZeroTab.HOME
    }
    ...
    Box(
        ...
        BoxWithConstraints(
            ...
            // --- Capsule blobs (opaque, anti-aliased) ----------------------
            ...
            Row(
                ...
                ZeroNavItem(
                    imageVector = ZeroIcons.home,
                    label = "خانه",
                    selected = selectedTab == ZeroTab.HOME,
                    inactiveColor = NAV_INACTIVE,
                    onClick = { Store.view = "home" },
                    modifier = Modifier.weight(1f)
                )
                ZeroNavItem(
                    imageVector = ZeroIcons.locations,
                    label = "سرورها",
                    selected = selectedTab == ZeroTab.LOCATIONS,
                    inactiveColor = NAV_INACTIVE,
                ...
                ZeroNavItem(
                    ...
                    label = "تنظیمات",
                    selected = selectedTab == ZeroTab.SETTINGS,
                    inactiveColor = NAV_INACTIVE,
...
private val NECK_THETA_END = (Math.PI / 6).toFloat()  // 30° attach angle when stretched
